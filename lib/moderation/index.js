...
// Match the model the existing /api/ai/moderate-image route uses — that
// one has been working in prod, claude-haiku-4-5 was returning 400s on
// images near the API's payload limit and a swastika slipped through
// because the upload soft-flagged on upstream_error and the post route
// trusted the URL anyway. See urlIsApproved() below for the post-side gate.
...
async function classifyAnthropic({ buffer, mediaType }) {
  ...
  try {
    ...
    if (!r.ok) {
      console.warn('[moderation] anthropic non-OK:', r.status);
      return { classifier: 'anthropic', action: 'soft_flag', reasons: ['upstream_error'], raw: { status: r.status } };
    }
